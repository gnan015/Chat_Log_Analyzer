...
function render(data) {
  ...
  chatSummary.textContent = makeSummary(valid);
...
function makeSummary(valid) {
  ...
  for (const item of valid) {
    ...
    if (!message || /^<media omitted>$/i.test(message)) continue;
    const person = highlights.get(item.user) || [];
    if (person.length < 2 && !person.includes(message)) person.push(message);
    highlights.set(item.user, person);
  }
  const recap = [...highlights.entries()]
    .filter(([, messages]) => messages.length)
    .slice(0, 4)
    .map(([user, messages]) => `${user} said “${messages.map(shorten).join('” and “')}”`)
    .join('. ');
  return recap ? `Conversation recap: ${recap}.` : 'The chat contains no text messages that can be summarized.';
}
function shorten(message) { return message.length > 140 ? `${message.slice(0, 137)}...` : message; }
